...
pub fn build_append_delegation_request(
    ...
    match new_policies.iter().position(|p| {
        p.target.resource.resource_type == new_policy.target.resource.resource_type
            && p.target.resource.identifiers == new_policy.target.resource.identifiers
    }) {
        Some(idx) => {
            new_policies.splice(idx..idx + 1, vec![new_policy]);
        }
        None => {
            new_policies.push(new_policy);
        }
    };
